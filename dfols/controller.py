...
class ExitInformation(object):
    ...
    def able_to_do_restart(self):
        if self.flag in [
            ...
            return (
                "sufficiently small" not in self.msg
            )  # restart for rho=rhoend and noise level termination
...
class Controller(object):
    def __init__(
        ...
        self.model = Model(
            ...
            abs_tol=params("model.abs_tol"),
            rel_tol=params("model.rel_tol"),
        ...
        self.num_slow_iters = (
            0  # how many consecutive slow iterations have we had so far
        )
    ...
    def initialise_coordinate_directions(
        self, number_of_samples, num_directions, params
    ):
        if self.do_logging:
        ...
        if self.model.projections:
            ...
            k = 0
            slctr = np.random.randint(
                0, 1 + 1, self.n()
            )  # generate rand binary "selector" array
            ...
            for k in range(0, self.n()):
                ...
                rvec_list, f_list, num_samples_run, exit_info = self.evaluate_objective(
                ...
                self.model.change_point(
                    k + 1, x - self.model.xbase, rvec_list[0, :]
        ...
        for k in range(1, num_directions + 1):
            ...
            if 1 <= k < self.n() + 1:  # first step along coord directions
                ...
                stepa = (
                    self.delta if not at_upper_boundary[dirn] else -self.delta
                )  # take a +delta step if at lower, -delta if at upper
                stepb = None
                xpts_added[
                    k, dirn
                ] = stepa  # set new (relative) point to the step since we haven't done any moving, so relative point is all zeros.

            elif (
                self.n() + 1 <= k < 2 * self.n() + 1
            ):  # second step along coord directions
            ...
            # Otherwise, add new results (increments model.npt_so_far)
            self.model.change_point(
                k, x - self.model.xbase, rvec_list[0, :]
            )  # expect step, not absolute x
            for i in range(1, num_samples_run):
            ...
            if self.n() + 1 <= k < 2 * self.n() + 1:
                ...
                if (
                    stepa * stepb < 0.0
                    and self.model.fval[k] < self.model.fval[k - self.n()]
                ):
    ...
    def initialise_random_directions(self, number_of_samples, num_directions, params):
        ...
        if params("init.random_directions_make_orthogonal"):
            dirns = random_orthog_directions_within_bounds(
                num_directions, self.delta, self.model.sl - xopt, self.model.su - xopt
            )
        else:
            dirns = random_directions_within_bounds(
                num_directions, self.delta, self.model.sl - xopt, self.model.su - xopt
            )
        ...
        if params("init.run_in_parallel"):
            ...
            for ndirns in range(num_directions):
                new_point = (
                    xopt + dirns[ndirns, :]
                )  # alway base move around best value so far

                # Evaluate objective
                x = self.model.as_absolute_coordinates(new_point)
                eval_obj_results.append(
                    self.evaluate_objective(x, number_of_samples, params)
                )
            ...
            for ndirns in range(num_directions):
                ...
                # Otherwise, add new results (increments model.npt_so_far)
                self.model.change_point(
                    1 + ndirns, x - self.model.xbase, rvec_list[0, :]
                )  # expect step, not absolute x
                for i in range(1, num_samples_run):
    ...
    def add_new_direction_while_growing(
        self, number_of_samples, params, min_num_steps=0
    ):
        ...
        xopt = self.model.xopt()
        dirns = random_directions_within_bounds(
            num_steps, step_length, self.model.sl - xopt, self.model.su - xopt
        )
        # Make direction orthogonal
        Y = self.model.xpt_directions(
            include_kopt=False
        ).T  # columns are the current set of directions
        Q, R = LA.qr(
            Y, mode="economic"
        )  # columns of Q are orthonormal basis for current set of directions
        ...
        for j in range(num_steps):
            xnew = (
                self.model.xopt() + (step_length / LA.norm(dirns[j, :])) * dirns[j, :]
            )
            ...
            if self.model.npt() < self.model.num_pts:  # still growing
                ...
                kmin, linalg_error = self.choose_point_to_replace(
                    xnew - self.model.xopt(), skip_kopt=True
                )

                if linalg_error:
                    exit_info = ExitInformation(
                        EXIT_LINALG_ERROR,
                        "Singular matrix when finding kmin (in main loop)",
                    )
                    return exit_info  # return & quit

            # Otherwise, add new results
            self.model.change_point(
                kmin, xnew, rvec_list[0, :]
            )  # expect step, not absolute x
            for i in range(1, num_samples_run):
    ...
    def get_new_direction_for_growing(self, step_length):
        # Step from xopt along a random direction orthogonal to other yt (or multiple mutually orthogonal steps)
        xopt = self.model.xopt()
        dirn = random_directions_within_bounds(
            1, step_length, self.model.sl - xopt, self.model.su - xopt
        )[0, :]
        # Make direction orthogonal
        Y = self.model.xpt_directions(
            include_kopt=False
    ...
    def trust_region_step(self, params):
        ...
        if self.model.projections:
            d, gnew, crvmin = ctrsbox(
                self.model.xopt(abs_coordinates=True),
                gopt,
                H,
                self.model.projections,
                self.delta,
                d_max_iters=params("dykstra.max_iters"),
                d_tol=params("dykstra.d_tol"),
            )
        else:
            d, gnew, crvmin = trsbox(
                self.model.xopt(), gopt, H, self.model.sl, self.model.su, self.delta
            )
    ...
    def geometry_step(self, knew, adelt, number_of_samples, params):
        ...
        try:
            ...
            if self.model.projections:
                ...
                xnew = trsbox_geometry(
                    self.model.xopt(),
                    c,
                    g,
                    np.minimum(self.model.sl, 0.0),
                    np.maximum(self.model.su, 0.0),
                    adelt,
                )
        except LA.LinAlgError:
            exit_info = ExitInformation(
                EXIT_LINALG_ERROR, "Singular matrix encountered in geometry step"
            )
        ...
        )  # save here, to calculate predicted value from geometry step
        ...
        self.diffs = [
            abs(pred_reduction - actual_reduction),
            self.diffs[0],
            self.diffs[1],
        ]
        return None  # exit_info = None

    def check_and_fix_geometry(
        self, distsq_thresh, update_delta, number_of_samples, params
    ...
    def evaluate_objective(self, x, number_of_samples, params):
        ...
        for i in range(number_of_samples):
            ...
            rvec_list[i, :], f_list[i] = eval_least_squares_objective(
                self.objfun,
                remove_scaling(x, self.scaling_changes),
                args=self.args,
                eval_num=self.nf,
                pt_num=self.nx,
        ...
        if (
            num_samples_run > 0
            and sumsq(np.mean(rvec_list[:num_samples_run, :], axis=0))
            <= self.model.min_objective_value()
        ):
    ...
    def choose_point_to_replace(self, d, skip_kopt=True):
        ...
        try:
            cs, gs = self.model.lagrange_gradient(
                k=None
            )  # find all Lagrange polynomials for k in range(self.model.npt())
        except LA.LinAlgError:
            exit_info = ExitInformation(
                EXIT_LINALG_ERROR, "Singular matrix when choosing point to replace"
            )
    ...
    def calculate_ratio(self, current_iter, rvec_list, d, gopt, H):
        ...
        self.diffs = [
            abs(actual_reduction - pred_reduction),
            self.diffs[0],
            self.diffs[1],
        ]
        ...
        if pred_reduction < 0.0:
            if (
                len(self.model.projections) > 1
            ):  # if we are using multiple projections, only warn since likely due to constraint intersection
                exit_info = ExitInformation(
                    EXIT_TR_INCREASE_WARNING,
                    "Either multiple constraints are active or trust region step gave model increase",
                )
            else:
                exit_info = ExitInformation(
                    EXIT_TR_INCREASE_ERROR,
                    "Either rust region step gave model increase",
                )
    ...
    def terminate_from_slow_iterations(self, current_iter, params):
        if len(self.last_iters_step_taken) <= params("slow.history_for_slow"):
            ...
            self.last_fopts_step_taken = self.last_fopts_step_taken[1:] + [
                self.model.fopt()
            ]
            this_iter_slow = (
                log(self.last_fopts_step_taken[0]) - log(self.model.fopt())
        ...
        if this_iter_slow:
            ...
            if self.do_logging:
                module_logger.info(
                    "Slow iteration (%g consecutive so far, max allowed %g)"
    ...
    def soft_restart(
        ...
        ok_to_do_restart = (
            nruns_so_far - self.last_successful_run
            < params("restarts.max_unsuccessful_restarts")
        ) and (self.nf < self.maxfun)
        ...
        if not ok_to_do_restart:
            ...
            if nruns_so_far - self.last_successful_run >= params(
        ...
        if x_in_abs_coords_to_save is not None:
            assert (
                rvec_to_save is not None
            ), "Soft restart: specified x_to_save but not rvec_to_save"
            assert (
                nsamples_to_save is not None
            ), "Soft restart: specified x_to_save but not nsamples_to_save"
            self.model.save_point(
                x_in_abs_coords_to_save,
                rvec_to_save,
                nsamples_to_save,
                x_in_abs_coords=True,
            )
        self.model.save_point(
            self.model.xopt(abs_coordinates=True),
            self.model.ropt(),
            self.model.nsamples[self.model.kopt],
            x_in_abs_coords=True,
        )

        if self.do_logging:
            module_logger.info(
                "Soft restart [currently, f = %g after %g function evals]"
                % (self.model.fopt(), self.nf)
            )
        ...
        if params("restarts.soft.move_xk"):
            ...
            closest_points = closest_points[
                1 : params("restarts.soft.num_geom_steps") + 1
            ]
        ...
        if params("restarts.increase_npt") and self.model.npt() < params(
            "restarts.max_npt"
        ):
            num_pts_to_add = min(
                params("restarts.increase_npt_amt"),
                params("restarts.max_npt") - self.model.npt(),
            )
            ...
            xopt = self.model.xopt()
            dirns = random_directions_within_bounds(
                num_pts_to_add, self.delta, self.model.sl - xopt, self.model.su - xopt
            )
            for i in range(num_pts_to_add):
                xnew = (
                    self.model.xopt() + dirns[i, :]
                )  # always base move around best value so far
                x = self.model.as_absolute_coordinates(xnew)
                rvec_list, f_list, num_samples_run, exit_info = self.evaluate_objective(
                    x, number_of_samples, params
                )
                ...
                # Otherwise, add new results
                self.model.add_new_point(
                    xnew, rvec_list[0, :]
                )  # expect step, not absolute x
                for i in range(1, num_samples_run):
                    self.model.add_new_sample(
                        self.model.npt() - 1, rvec_extra=rvec_list[i, :]
                    )

            if self.do_logging:
                module_logger.info(
                    "Soft restart: added %g new directions, npt is now %g"
                    % (num_pts_to_add, self.model.npt())
                )
    ...
    def move_furthest_points(self, number_of_samples, num_pts_to_move, params):
        ...
        all_sq_dist = self.model.distances_to_xopt()[: self.model.npt()]
        furthest_points = np.argsort(all_sq_dist)[
            ::-1
        ]  # indices from furthest to closest (last is kopt)
    ...
    def all_values_within_noise_level(self, params):
        ...
        if params("noise.additive_noise_level") is not None:
            add_noise = params("noise.scale_factor_for_quit") * params(
                "noise.additive_noise_level"
            )
            for k in range(self.model.npt()):
                all_fvals_within_noise = all_fvals_within_noise and (
                    self.model.fval[k]
                    <= self.model.fopt() + add_noise / sqrt(self.model.nsamples[k])
                )
        else:  # noise_level_multiplicative
            ratio = 1.0 + params("noise.scale_factor_for_quit") * params(
                "noise.multiplicative_noise_level"
            )
            for k in range(self.model.npt()):
                this_ratio = (
                    self.model.fval[k] / self.model.fopt()
                )  # fval_opt strictly positive (would have quit o/w)
                all_fvals_within_noise = all_fvals_within_noise and (
                    this_ratio <= ratio / sqrt(self.model.nsamples[k])
                )
        return all_fvals_within_noise

    def move_furthest_points_momentum(
        self, d, number_of_samples, num_pts_to_move, params
    ):
        all_sq_dist = self.model.distances_to_xopt()[: self.model.npt()]
        furthest_points = np.argsort(all_sq_dist)[
            ::-1
        ...
        xopt = self.model.xopt()
        dirns = random_directions_within_bounds(
            num_pts_to_move, self.delta, self.model.sl - xopt, self.model.su - xopt
        )
        ...
        for i in range(min(num_pts_to_move, len(furthest_points) - 1)):
            ...
            if flipped:
                fixed_dirn = np.maximum(
                    np.minimum(dirns[i, :], self.model.su - self.model.xopt()),
                    self.model.sl - self.model.xopt(),
                )
                if np.linalg.norm(fixed_dirn) < 1e-3 * self.delta:
                    dirns[i, :] = -dirns[i, :]
            xnew = np.maximum(
                np.minimum(self.model.xopt() + dirns[i, :], self.model.su),
                self.model.sl,
            )
            x = self.model.as_absolute_coordinates(xnew)
            rvec_list, f_list, num_samples_run, exit_info = self.evaluate_objective(
