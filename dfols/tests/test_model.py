...
class TestAddValues(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            array_compare(model.xopt(abs_coordinates=True), x0),
            "Wrong xopt after initialisation",
        )
        self.assertTrue(
            array_compare(model.ropt(), rosenbrock(x0)),
            "Wrong ropt after initialisation",
        )
        self.assertAlmostEqual(
            model.fopt(), sumsq(rosenbrock(x0)), "Wrong fopt after initialisation"
        )
        ...
        self.assertTrue(
            array_compare(model.xopt(abs_coordinates=True), x1), "Wrong xopt after x1"
        )
        self.assertTrue(
            array_compare(model.ropt(), rosenbrock(x1)), "Wrong ropt after x1"
        )
        self.assertAlmostEqual(
            model.fopt(), sumsq(rosenbrock(x1)), "Wrong fopt after x1"
        )
        # Now add worse point
        x2 = np.array([2.0, 0.9])
        rvec = rosenbrock(x2)
        ...
        self.assertTrue(
            array_compare(model.xpt(0, abs_coordinates=True), x0),
            "Wrong xpt(0) after x2",
        )
        self.assertTrue(
            array_compare(model.xpt(1, abs_coordinates=True), x1),
            "Wrong xpt(1) after x2",
        )
        self.assertTrue(
            array_compare(model.xpt(2, abs_coordinates=True), x2),
            "Wrong xpt(2) after x2",
        )
        self.assertTrue(
            array_compare(model.xopt(abs_coordinates=True), x1), "Wrong xopt after x2"
        )
        self.assertTrue(
            array_compare(model.ropt(), rosenbrock(x1)), "Wrong ropt after x2"
        )
        self.assertAlmostEqual(
            model.fopt(), sumsq(rosenbrock(x1)), "Wrong fopt after x2"
        )
        # Now add best point (but don't update kopt)
        x3 = np.array([1.0, 1.0])
        rvec = rosenbrock(x3)
        model.change_point(
            0, x3 - model.xbase, rvec, allow_kopt_update=False
        )  # full: overwrite x0
        self.assertEqual(model.npt(), 3, "Wrong npt after x3")
        self.assertTrue(
            array_compare(model.xopt(abs_coordinates=True), x1), "Wrong xopt after x3"
        )
        self.assertTrue(
            array_compare(model.ropt(), rosenbrock(x1)), "Wrong ropt after x3"
        )
        self.assertAlmostEqual(
            model.fopt(), sumsq(rosenbrock(x1)), "Wrong fopt after x3"
        )
        self.assertTrue(
            array_compare(
                model.xopt(abs_coordinates=True),
...
class TestSwap(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            array_compare(model.xpt(0, abs_coordinates=True), x2),
        ...
        self.assertTrue(
            array_compare(model.xpt(2, abs_coordinates=True), x0),
            "Wrong xpt(2) after swap 1",
        )
        self.assertTrue(
            array_compare(model.xopt(abs_coordinates=True), x1),
        ...
        self.assertTrue(
            array_compare(model.xpt(1, abs_coordinates=True), x0),
            "Wrong xpt(1) after swap 2",
        )
        self.assertTrue(
            array_compare(model.xpt(2, abs_coordinates=True), x1),
...
class TestBasicManipulation(unittest.TestCase):
    def runTest(self):
        ...
        model = Model(npt, x0, rosenbrock(x0), xl, xu, [], 1)
        self.assertTrue(
            array_compare(model.sl, xl - x0), "Wrong sl after initialisation"
        )
        self.assertTrue(
            array_compare(model.su, xu - x0), "Wrong su after initialisation"
        )
        x1 = np.array([1.0, 0.9])
        ...
        self.assertTrue(
            array_compare(model.ropt(), 2 * rosenbrock(x0) / 52),
            "Wrong ropt after bad resampling",
        )
        self.assertAlmostEqual(
            model.fopt(),
            sumsq(2 * rosenbrock(x0) / 52),
        ...
        for i in range(3):
            self.assertTrue(
                array_compare(dirns_old[i, :], dirns_new[i, :]),
                "Wrong dirn %i after shift base" % i,
            )
        self.assertTrue(
            array_compare(model.sl, xl - x0 - d), "Wrong sl after shift base"
        )
        self.assertTrue(
            array_compare(model.su, xu - x0 - d), "Wrong su after shift base"
        )
        # save_point and get_final_results
        model.change_point(
            0, x0 - model.xbase, rosenbrock(x0)
        )  # revert after resampling
        model.change_point(
            1, x1 - model.xbase, rosenbrock(x1)
        )  # revert after resampling
        ...
        self.assertIsNone(
            model.nsamples_save, "nsamples_save not none after initialisation"
        )
        ...
        self.assertTrue(
            array_compare(model.rsave, rosenbrock(x0)), "Wrong rsave after saving"
        )
        self.assertAlmostEqual(
            model.fsave, sumsq(rosenbrock(x0)), "Wrong fsave after saving"
        )
        ...
        self.assertTrue(
            array_compare(rvec, rosenbrock(x1)), "Wrong final rvec after saving"
        )
        ...
        model.save_point(
            x2 - model.xbase, np.array([0.0, 0.0]), 2, x_in_abs_coords=False
        )
        self.assertTrue(array_compare(model.xsave, x2), "Wrong xsave after saving 2")
        self.assertTrue(
            array_compare(model.rsave, np.array([0.0, 0.0])),
            "Wrong rsave after saving 2",
        )
...
class TestMinObjValue(unittest.TestCase):
    def runTest(self):
        ...
        model.change_point(2, x2 - model.xbase, rosenbrock(x2))
        self.assertAlmostEqual(
            model.min_objective_value(), 1e-12, "Wrong min obj value"
        )
        ...
        self.assertAlmostEqual(
            model.min_objective_value(), 1.0, "Wrong min obj value 3"
        )
        model = Model(npt, x0, rosenbrock(x0), xl, xu, [], 1, abs_tol=1.0, rel_tol=1e-2)
        self.assertAlmostEqual(
            model.min_objective_value(), 1.0, "Wrong min obj value 4"
        )
...
class TestInterpMatrixSVD(unittest.TestCase):
    def runTest(self):
        ...
        A_expect[2, 1:] = x2 - x1
        A_after_scaling = np.dot(
            np.diag(1.0 / left_scaling), np.dot(A, np.diag(1.0 / right_scaling))
        )
        ...
        (
            ...
            linalg_resid,
            ls_interp_cond_num,
        ) = model.interpolate_mini_models_svd()
        ...
        self.assertTrue(
            array_compare(model.model_const, rosenbrock(model.xbase), thresh=1e-10),
            "Wrong constant term",
        )
        self.assertTrue(
            array_compare(
                model.model_value(
                    x0 - model.xbase, d_based_at_xopt=False, with_const_term=True
                ),
                rosenbrock(x0),
                thresh=1e-10,
            ),
        ...
        self.assertTrue(
            array_compare(
                model.model_value(
                    x1 - model.xbase, d_based_at_xopt=False, with_const_term=True
                ),
                rosenbrock(x1),
                thresh=1e-10,
            ),
        ...
        self.assertTrue(
            array_compare(
                model.model_value(
                    x2 - model.xbase, d_based_at_xopt=False, with_const_term=True
                ),
                rosenbrock(x2),
                thresh=1e-10,
            ),
        ...
        self.assertTrue(
            array_compare(
                model.model_value(x2 - x1, d_based_at_xopt=True, with_const_term=True),
                rosenbrock(x2),
                thresh=1e-10,
            ),
            "Wrong x2 (from xopt)",
        ...
        self.assertTrue(
            array_compare(
                model.model_value(x2 - x1, d_based_at_xopt=True, with_const_term=False),
                rosenbrock(x2) - rosenbrock(model.xbase),
                thresh=1e-10,
            ),
            "Wrong x2 (no constant)",
        )
        self.assertTrue(
            array_compare(
                model.model_value(
                    x2 - model.xbase, d_based_at_xopt=False, with_const_term=False
                ),
                rosenbrock(x2) - rosenbrock(model.xbase),
                thresh=1e-10,
            ),
            "Wrong x2 (no constant v2)",
        )
        g, H = model.build_full_model()
        r = rosenbrock(x1)
        J = model.model_jac
        self.assertTrue(
            array_compare(g, 2.0 * np.dot(J.T, r), thresh=1e-10), "Bad gradient"
        )
...
class TestGeomSystem(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            abs(c0 + np.dot(g0, x0 - xopt) - 1.0) < lag_thresh, "Bad L0(x0)"
        )
        self.assertTrue(
            abs(c0 + np.dot(g0, x1 - xopt) - 0.0) < lag_thresh, "Bad L0(x1)"
        )
        self.assertTrue(
            abs(c0 + np.dot(g0, x2 - xopt) - 0.0) < lag_thresh, "Bad L0(x2)"
        )
        c1, g1 = model.lagrange_gradient(1)
        # print(c1 + np.dot(g1, x0 - xopt), c1, g1)
        self.assertTrue(
            abs(c1 + np.dot(g1, x0 - xopt) - 0.0) < lag_thresh, "Bad L1(x0)"
        )
        self.assertTrue(
            abs(c1 + np.dot(g1, x1 - xopt) - 1.0) < lag_thresh, "Bad L1(x1)"
        )
        self.assertTrue(
            abs(c1 + np.dot(g1, x2 - xopt) - 0.0) < lag_thresh, "Bad L1(x2)"
        )
        c2, g2 = model.lagrange_gradient(2)
        self.assertTrue(
            abs(c2 + np.dot(g2, x0 - xopt) - 0.0) < lag_thresh, "Bad L2(x0)"
        )
        self.assertTrue(
            abs(c2 + np.dot(g2, x1 - xopt) - 0.0) < lag_thresh, "Bad L2(x1)"
        )
        self.assertTrue(
            abs(c2 + np.dot(g2, x2 - xopt) - 1.0) < lag_thresh, "Bad L2(x2)"
        )
...
class TestFullRankSVD(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            10 * sng_vals[-1] < sng_vals2[-1], "New singular value too small"
        )
...
class TestPoisedness(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            abs(model.poisedness_constant(delta) - 1.0 - sqrt(2.0)) < 1e-6,
            "Poisedness constant wrong",
        )
...
class TestAddPoint(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            array_compare(model.xpt(3, abs_coordinates=True), x3),
            "Wrong new point after x3",
        )
        self.assertTrue(
            array_compare(model.rvec(3), rosenbrock(x3)), "Wrong rvec after x3"
        )
        ...
        self.assertTrue(
            array_compare(model.xpt(4, abs_coordinates=True), x4),
            "Wrong new point after x4",
        )
        self.assertTrue(
            array_compare(model.rvec(4), rosenbrock(x4)), "Wrong rvec after x4"
        )
...
class TestRegression(unittest.TestCase):
    def runTest(self):
        ...
        A_after_scaling = np.dot(
            np.diag(1.0 / left_scaling), np.dot(A2, np.diag(1.0 / right_scaling))
        )  # undo scaling
        # print(A_for_interp)
        ...
        self.assertTrue(
            array_compare(model.model_const, np.array([c])), "Wrong constant term"
        )
