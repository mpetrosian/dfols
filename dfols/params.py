...
class ParameterList(object):
    def __init__(self, n, npt, maxfun, objfun_has_noise=False):
        ...
        ] = 0.1  # 0.1 in DFBOLS, 1e-3 in BOBYQA
        self.params[
            "general.safety_step_thresh"
        ] = 0.5  # safety step called if ||d|| <= thresh * rho
        self.params["general.check_objfun_for_overflow"] = True
        # Initialisation
        self.params["init.random_initial_directions"] = (
            True if npt > (n + 1) * (n + 2) // 2 else False
        )  # only if needed
        self.params[
            "init.run_in_parallel"
        ] = False  # only available for random directions at the moment
        self.params[
            "init.random_directions_make_orthogonal"
        ] = True  # although random > orthogonal, avoid for init
        # Interpolation
        self.params["interpolation.precondition"] = True
        self.params[
            "interpolation.throw_error_on_nans"
        ] = False  # throw numpy.linalg.LinAlgError if interpolating to nan data?
        # Logging
        ...
        ] = 0  # number of extra points to move in a successful TR iteration
        self.params[
            "regression.increase_num_extra_steps_with_restart"
        ] = 0  # number of extra steps to add each restart
        self.params[
            "regression.momentum_extra_steps"
        ] = False  # use momentum (False) or geometry (True) to do steps
        # Restarts
        ...
        ] = 1.0  # how much to decrease rhoend by after each restart
        ...
        self.params[
            "growing.full_rank.svd_scale_factor"
        ] = 1.0  # floor sing vals at svd_scale_factor*s[r]
        self.params[
            "growing.full_rank.min_sing_val"
        ] = 1e-6  # absolute floor on singular values
        self.params[
            "growing.full_rank.svd_max_jac_cond"
        ] = 1e8  # maximum condition number of Jacobian
        self.params[
            "growing.perturb_trust_region_step"
        ] = False  # add random direction onto TRS solution?
        # Dykstra's algorithm
