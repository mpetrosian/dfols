...
class Model(object):
    def __init__(
        ...
        assert x0.shape == (n,), "x0 has wrong shape (got %s, expect (%g,))" % (
            str(x0.shape),
            n,
        )
        assert xl.shape == (n,), "xl has wrong shape (got %s, expect (%g,))" % (
            str(xl.shape),
            n,
        )
        assert xu.shape == (n,), "xu has wrong shape (got %s, expect (%g,))" % (
            str(xu.shape),
            n,
        )
        assert r0.shape == (m,), "r0 has wrong shape (got %s, expect (%g,))" % (
            str(r0.shape),
            m,
        )
    ...
    def xpt(self, k, abs_coordinates=False):
        ...
        if not abs_coordinates:
            ...
            return self.xbase + np.minimum(
                np.maximum(self.sl, self.points[k, :]), self.su
            )
    ...
    def xpt_directions(self, include_kopt=True):
        ...
        dirns = np.zeros(
            (ndirs, self.n())
        )  # vector of directions xpt - xopt, excluding for xopt
        xopt = self.xopt()
    ...
    def add_new_point(self, x, rvec):
        self.points = np.append(
            self.points, x.reshape((1, self.n())), axis=0
        )  # append row to xpt
        self.fval_v = np.append(
            self.fval_v, rvec.reshape((1, self.m())), axis=0
        )  # append row to fval_v
        f = np.dot(rvec, rvec)
    ...
    def save_point(self, x, rvec, nsamples, x_in_abs_coords=True):
        ...
        if self.fsave is None or f <= self.fsave:
            self.xsave = (
                x.copy() if x_in_abs_coords else self.as_absolute_coordinates(x)
            )
    ...
    def get_final_results(self):
        ...
        if (
            self.fsave is None or self.fopt() <= self.fsave
        ):  # optimal has changed since xsave+fsave were last set
            return (
                self.xopt(abs_coordinates=True).copy(),
                self.ropt().copy(),
                self.fopt(),
                self.model_jac.copy(),
                self.nsamples[self.kopt],
            )
        else:
            return (
                self.xsave.copy(),
    ...
    def interpolation_matrix(self):
        ...
        if self.precondition:
            approx_delta = sqrt(
                np.max(self.distances_to_xopt())
            )  # largest distance to xopt ~ delta
        else:
            approx_delta = 1.0

        W[:, 0] = 1.0
        W[:, 1:] = (
            self.xpt_directions(include_kopt=True) / approx_delta
        )  # rows are (yt-xk)
    ...
    def solve_geom_system(self, rhs):
        ...
        col_scale = lambda A, scale: (
            A.T * scale
        ).T  # Uses the trick that A*x scales the 0th column of A by x[0], etc.
        ...
        if self.factorisation_current:
            if self.qr_of_transpose:
                ...
                Rb = LA.solve_triangular(
                    self.R, col_scale(rhs, self.left_scaling), trans="T"
                )  # R.T \ rhs
                return col_scale(
                    np.dot(self.Q, Rb), self.right_scaling
            ...
            return col_scale(
                LA.lstsq(W, col_scale(rhs * left_scaling))[0], right_scaling
            )

    def interpolate_mini_models_svd(
        self,
        verbose=False,
        ...
        ls_interp_cond_num = (
            np.linalg.cond(W) if verbose else 0.0
        )  # scipy.linalg does not have condition number!
        ...
        if np.any(np.isnan(rhs)) and throw_error_on_nans:
            if self.do_logging:
                module_logger.warning(
                    "model.interpolate_mini_models_svd: NaNs encountered in objective evaluations, raising error"
                )
        ...
        if make_full_rank:
            ...
            k = min(self.n(), self.m())
            r = min(
                self.npt_so_far - 1, self.n(), self.m()
            )  # current number of directions (i.e. rank of J)
        ...
        if verbose:
            for k in range(self.npt()):
                r_pred = self.model_value(
                    self.xpt(k), d_based_at_xopt=False, with_const_term=True
                )
    ...
    def build_full_model(self):
        ...
        # Centred around xopt
        r = self.model_const + np.dot(
            self.model_jac, self.xopt()
        )  # constant term (for inexact interpolation)
        J = self.model_jac
    ...
    def lagrange_gradient(self, k=None, factorise_first=True):
        ...
        if k is not None:
            ...
            return (
                cs,
                gs,
            )  # constant terms in each entry and gradient terms in each col [all based at xopt]
